...
import { hadithDetails, hadithImportanceList, hadithFawaedList} from './hadith13-data';

@Component({
  selector: 'app-hadith-13',
  standalone: true,
  imports: [ CommonModule, RouterModule, FooterInfoComponent, NextBeforeSurahMenyComponent, ZoomControlsComponent ],
  templateUrl: './hadith-13.component.html',
  styleUrl: './hadith-13.component.css'
})

export class Hadith13Component implements OnInit, OnDestroy {
...
ngOnInit() {
  // تعيين عنوان الصفحة
  this.titleService.setTitle('باب التوبة. الحديث الثالث عشر - شرح رياض الصالحين');
  ...
  this.metaService.updateTag({ 
    property: 'og:title', 
    content: 'باب التوبة. الحديث الثالث عشر - شرح رياض الصالحين'
  });
