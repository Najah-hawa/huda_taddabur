...
import { hadithDetails, hadithImportanceList, hadithFawaedList} from './hadith11-data';

@Component({
   selector: 'app-hadith-11',
  standalone: true,
  imports: [ CommonModule, RouterModule, FooterInfoComponent, NextBeforeSurahMenyComponent, ZoomControlsComponent ],
  templateUrl: './hadith-11.component.html',
  styleUrl: './hadith-11.component.css'
})

export class Hadith11Component implements OnInit, OnDestroy {
...
ngOnInit() {
  this.titleService.setTitle('الحديث الحادي عشر: كتابة الحسنات والسيئات - شروح الأربعين النووية');
  
  this.metaService.updateTag({
    name: 'description',
    content: 'شرح وتدبر الحديث الحادي عشر عن ابن عباس (كتابة الحسنات والسيئات)، وبيان سعة فضل الله وكرمه في مضاعفة الحسنات، وكتابة الهم بالخير، وعدم مضاعفة السيئات.'
  });
  
  this.metaService.updateTag({
    name: 'keywords',
    content: 'كتابة الحسنات والسيئات, الحديث الحادي عشر, ابن عباس, ترجمان القرآن, الهم بالحسنة, مضاعفة الحسنات, شرح الحديث, الأربعون النووية'
  });
  
  this.metaService.updateTag({ 
    property: 'og:title', 
    content: 'الحديث الحادي عشر: كتابة الحسنات والسيئات - تدبر تفاعلي' 
  });
  
  this.metaService.updateTag({ 
    property: 'og:description', 
    content: 'اقرأ واستمع إلى متن الحديث الحادي عشر مع التظليل التزامني، تفسير المفردات (همّ، ضعف)، وترجمة حبر الأمة عبد الله بن عباس وعظم فضل الله على الأمة.' 
  });
