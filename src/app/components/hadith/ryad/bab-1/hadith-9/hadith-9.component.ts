...
import { hadithDetails, hadithImportanceList, hadithFawaedList} from './hadith9-data';

@Component({
   selector: 'app-hadith-9',
  standalone: true,
  imports: [ CommonModule, RouterModule, FooterInfoComponent, NextBeforeSurahMenyComponent, ZoomControlsComponent ],
  templateUrl: './hadith-9.component.html',
  styleUrl: './hadith-9.component.css'
})

export class Hadith9Component implements OnInit, OnDestroy {
...
ngOnInit() {
  this.titleService.setTitle('الحديث التاسع: خطورة قتال المسلمين - شروح الأربعين النووية');
  
  this.metaService.updateTag({
    name: 'description',
    content: 'شرح وتدبر حديث (إذا التقى المسلمان بسيفيهما فالقاتل والمقتول في النار) عن أبي بكرة، وبيان خطورة الاقتتال وعقوبة العزم والحرص على المعصية.'
  });
  
  this.metaService.updateTag({
    name: 'keywords',
    content: 'إذا التقى المسلمان بسيفيهما, القاتل والمقتول في النار, أبو بكرة, نفيع بن الحارث, عقوبة العزم على المعصية, قتال المسلمين, شرح الحديث, الأربعون النووية'
  });
  
  this.metaService.updateTag({ 
    property: 'og:title', 
    content: 'الحديث التاسع: خطورة قتال المسلمين - تدبر تفاعلي' 
  });
  
  this.metaService.updateTag({ 
    property: 'og:description', 
    content: 'اقرأ واستمع إلى الحديث الشريف مع التظليل التزامني، وتعرّف على ترجمة الصحابي أبي بكرة الثقفي، وحكم من وطن نفسه على المعصية وباشر أسبابها.' 
  });
