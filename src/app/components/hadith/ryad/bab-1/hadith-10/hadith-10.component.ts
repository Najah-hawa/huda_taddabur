...
import { hadithDetails, hadithImportanceList} from './hadith10-data';

@Component({
   selector: 'app-hadith-10',
  standalone: true,
  imports: [ CommonModule, RouterModule, FooterInfoComponent, NextBeforeSurahMenyComponent, ZoomControlsComponent ],
  templateUrl: './hadith-10.component.html',
  styleUrl: './hadith-10.component.css'
})

export class Hadith10Component implements OnInit, OnDestroy {
...
ngOnInit() {
  this.titleService.setTitle('الحديث العاشر: فضل صلاة الجماعة في المسجد - شروح الأربعين النووية');
  
  this.metaService.updateTag({
    name: 'description',
    content: 'شرح وتدبر الحديث العاشر عن أبي هريرة (صلاة الرجل في جماعة)، وبيان فضل صلاة الجماعة، ومضاعفة الأجور، وثواب انتظار الصلاة واستغفار الملائكة للمصلي.'
  });
  
  this.metaService.updateTag({
    name: 'keywords',
    content: 'صلاة الجماعة, صلاة الرجل في جماعة, الحديث العاشر, أبو هريرة, استغفار الملائكة, انتظار الصلاة, أحسن الوضوء, بضعا وعشرين درجة, شرح الحديث'
  });
  
  this.metaService.updateTag({ 
    property: 'og:title', 
    content: 'الحديث العاشر: فضل صلاة الجماعة - تدبر تفاعلي' 
  });
  
  this.metaService.updateTag({ 
    property: 'og:description', 
    content: 'اقرأ واستمع إلى متن الحديث العاشر مع التظليل التزامني، تفسير المفردات (ينهزه، بضعاً)، وبيان عظم ثواب ملازمة المصلى واستكثار دعاء الملائكة.' 
  });
